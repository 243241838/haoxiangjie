<template>
    <div class="app-container calendar-list-container">
        <el-tabs v-model="tabName" type="card" @tab-click="handleClick">
            <el-tab-pane label="电核记录" name="first">
                <div class="filter-container">
                    <h3>电核历史记录</h3>
                    <div v-if="dianheList">
                    <el-table :data="dianheList" border fit highlight-current-row style="width: 100%" >
                    ...
                    <div v-else>
                    <el-table :data="dianheList"  style="width: 100%;" height="120" >
                        <el-table-column align="center" label="序号" width="100">
                            <template scope="scope">
                                <span>{{ parseInt(scope.$index) + 1}}</span>
                            </template>
                ...
                <div class="filter-container">
                    <el-row>
                        <el-col :span="20" :offset="1">
                            <el-form ref="form" :model="form" label-width="110px" class="credit-aplly-form">
                                <el-form-item label="电核时间" required prop="name">
                                    <!-- <el-input v-model="listQuery.params.createTime"></el-input> -->
                                    <el-date-picker v-model="listQuery.params.createTime" type="datetime" placeholder="选择时间" :editable="false">
                                    </el-date-picker>
                                </el-form-item>
                                <el-form-item label="电核详情" required prop="name">
                                    <el-input type="textarea" v-model="listQuery.params.memo" style="width:500px;"></el-input>
                                </el-form-item>
                                <el-form-item>
            ...
            <el-tab-pane label="审批订单" name="second">
                <div class="filter-container">
                    <h2>额度调整</h2>
                    <el-row>
                        <el-col :span="20" :offset="1">
                            <el-form label-width="110px" class="credit-aplly-form">
                                <el-form-item label="额度调整为：" required prop="name">
                                    <el-input v-model="shenpilistQuery.params.acctSum" style="width:200px;" :placeholder="acctSum"></el-input>
                                </el-form-item>
                            </el-form>
                        </el-col>
                ...
                <div class="filter-container">
                    <h2>本次审核意见</h2>
                    <el-radio-group v-model="radio2" @change="getSettlement">
                        <el-radio :label="0">通过</el-radio>
                        <el-radio :label="1">驳回</el-radio>
                        <el-radio :label="2">拒绝加入黑名单</el-radio>
                    </el-radio-group>
                    <div v-if="radioValue===0" class="filter-container">
                        <el-row>
                            <el-col :span="20" :offset="1">
                                <el-form label-width="110px" class="credit-aplly-form">
                                    <el-form-item label="业务审核意见：" prop="name">
                                        <el-input v-model="shenpilistQuery.params.memo" type="textarea" style="width:500px;"></el-input>
                                    </el-form-item>
                                </el-form>
                            </el-col>
                        </el-row>
                    </div>
                    <div v-if="radioValue===1" class="filter-container">
                        <span>驳回原因:</span>
                        <!-- <el-select clearable style="width:200px;margin-bottom:20px;" class="filter-item" v-model="shenpilistQuery.params.refuseCode" placeholder="驳回原因">
                                                    <el-option v-for="item in creditAduitStatus" :key="item.code" :label="item.text" :value="item.code">
                                                    </el-option>
                                                </el-select> -->
                        <el-cascader clearable expand-trigger="hover" :show-all-levels="false" :options="options" v-model="shenpilistQuery.params.refuseCode" @change="handleChange" placeholder="驳回原因" style="width:200px;margin-bottom:20px;">
                        </el-cascader>
                        <el-row>
                            <el-form label-width="110px" class="credit-aplly-form">
                                <el-form-item label="业务审核意见：" prop="name">
                    ...
                    <div v-if="radioValue===2" class="filter-container">
                        <el-row>
                            <el-col :span="20" :offset="1">
                                <el-form label-width="110px" class="credit-aplly-form">
                                    <el-form-item label="拒绝加入黑名单原因：" prop="name">
                                        <el-input v-model="shenpilistQuery.params.refuseReason" type="textarea" style="width:500px;"></el-input>
                                    </el-form-item>
                                </el-form>
                            </el-col>
                    ...
                    <el-dialog title="确认" :visible.sync="dialogVisible" size="small">
                        <div>
                            <div v-if="this.radioValue===0">是否审批通过</div>
                            <div v-else-if="this.radioValue===1">是否确认审批不通过</div>
                            <div v-else-if="this.radioValue===2">是否确认拒绝并加入黑名单</div>
                        </div>
                        <span slot="footer" class="dialog-footer">
                            <el-button @click="cancel">重新审批</el-button>
            ...
            <el-tab-pane label="通讯录信息" name="fifth">
                <div class="filter-container">
                    <h3>通话记录明细</h3>
                    <el-table :data="dataList1" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
                        ...
                        <el-table-column align="center" label="来源">
                            <template scope="scope">
                                ...
                                <span v-else-if="scope.row.source===1">通讯录</span>
                            </template>
                        </el-table-column>
            ...
            <el-tab-pane label="征信信息" name="sixth">
...
export default {
    data() {
        return {
            ...
            listLoading1: false,
            dataList3: null,
            creditAduitStatus: null,
            listLoading3: false,
            form: { time: null, memo: null },
            //征信信息
            ...
            listQuery: { params: { url: 'authenticationApply/telAudit', applyNo: this.$route.query.applyNo, createTime: null, memo: null } },
            ...
            shenpilistQuery: { params: { url: 'authenticationApply/audit', applyNo: this.$route.query.applyNo, acctSum: null, refuseReason: '', status: null, refuseCode: '', memo: '' } },
            //shenpilist: { oldCredit: null, result: null, newCredit: null },
            //shenpiAdvice: { refuseReason: null, memo: null },
            shenpilist: null,
            shenpiAdvice: null,
            shenpiResult: null,
            //bankCardList: [{ realName: '', mobile: null, bankName: '', cardNo: null, idenNo: null, createTime: null }, { bankName: '', cardNo: null, createTime: null, creditLimit: null }],
            bankCardList: [{}, {}],
            applyNo: this.$route.params.applyNo,
            radio2: 3,
            radioValue: 0,
            auditForm: {
                auditOpinion: null,
            ...
            input3: '1',
            //审批下拉菜单
            auditOptionList: { params: { url: 'authenticationApply/getRefuseReasonList' } },
            //审批调整的额度
            auditAcctSum: { params: { url: 'authenticationApply/toAudit', applyNo: this.$route.query.applyNo } },
            acctSum: '',
            dialogVisible: false,
            options: null,
            //电核历史记录
            dianheQuery: { params: { url: 'authenticationApply/auditInfo', applyNo: this.$route.query.applyNo } },
            dianheList: null,
        };
    },
    created() {
    ...
    methods: {
        ...
        //电核历史记录获取
        getList() {
            config(this.dianheQuery.params).then(response => {
                if (response.respCode === '000000') {
                    this.dianheList = response.data.listTelAuditLogPo;
                }
            }).catch(err => {
            })
        ...
        //提交电核基本信息
        handleSubmit() {
            this.listQuery.params.createTime = parseTime(this.listQuery.params.createTime, '{y}-{m}-{d} {h}:{i}:{s}');

            if (!(this.listQuery.params.memo)) {
                this.$message({
                    message: '电核详情不能为空！',
                    type: 'warning'
                });
            ...
            if (!(this.listQuery.params.createTime)) {
                this.$message({
                    message: '电核时间不能为空！',
                    type: 'warning'
                });
            ...
            this.listQuery.params.applyNo = this.$route.query.applyNo;
            config(this.listQuery.params).then(response => {
                if (response.respCode === '000000') {
                    ...
                    // this.$router.push({ path: "/loanManage/apply" });
        ...
        //获取银行卡信息
        getbankcardList() {
            this.bankCardQuery.params.applyNo = this.$route.query.applyNo;
        ...
        //获取征信信息
        getCreditList() {
            this.creditQuery.params.applyNo = this.$route.query.applyNo;
        ...
        getSettlement(e) {
            this.radioValue = e;
            if (e === 0) {
                this.shenpilistQuery.params.status = 10;
                this.shenpilistQuery.params.refuseReason = "";
                this.shenpilistQuery.params.memo = "";
            }
            if (e === 1) {
                this.shenpilistQuery.params.status = -10;
                this.shenpilistQuery.params.refuseCode = "";
                this.shenpilistQuery.params.memo = "";
                this.shenpilistQuery.params.acctSum = this.acctSum;
            }
            if (e === 2) {
                this.shenpilistQuery.params.status = -20;
                this.shenpilistQuery.params.memo = "";
                this.shenpilistQuery.params.refuseCode = "";
                this.shenpilistQuery.params.acctSum = this.acctSum;
            }
        ...
        //提交审核信息
        submitAudit() {
            if (!this.shenpilistQuery.params.acctSum) {
                this.shenpilistQuery.params.acctSum = this.acctSum;
            }
            ...
            if (this.shenpilistQuery.params.acctSum > 3000) {
                this.$message({
                    message: '额度不能大于3000！',
                    type: 'warning'
                });
            ...
            if (this.radioValue === 1) {
                if (!this.shenpilistQuery.params.refuseCode) {
                    this.$message({
                        message: '请选择驳回原因！',
                        type: 'warning'
                    });
        ...
        //下拉选择项
        getOptionList() {
            config(this.auditOptionList.params).then(response => {
                if (response.respCode === '000000') {
        ...
        //获取审核调整的额度
        getAuditAcctSum() {
            config(this.auditAcctSum.params).then(response => {
                if (response.respCode === '000000') {
                    this.acctSum = response.data.acctSum + '';
                }
            }).catch(err => {
        ...
        //弹框确定按钮
        confirm() {
            if (this.shenpilistQuery) {
                this.shenpilistQuery.params.refuseCode = this.shenpilistQuery.params.refuseCode[1];
            }
            config(this.shenpilistQuery.params).then(response => {
                if (response.respCode === '000000') {
