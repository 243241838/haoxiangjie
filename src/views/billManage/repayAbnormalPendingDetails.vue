    ...
    <div class="app-container calendar-list-container">
        <el-tabs v-model="tabsName" type="card" @tab-click="handleClick">
            <el-tab-pane label="订单信息" name="first">
                        ...
                        <el-col :span="10" :offset="1">
                            <el-form label-width="110px" class="credit-aplly-form" v-if="lendOrderPo">
                                ...
                                <el-form-item label="放款状态：">
                                    <span>{{lendOrderPo.paymentStatus | paymentStatusFilter1}}</span>
                                </el-form-item>
                                <el-form-item label="放款方式：">
                                    <span>{{lendOrderPo.paySource | paySourceFilter}}</span>
                                </el-form-item>
                            </el-form>
                            <el-form label-width="110px" class="credit-aplly-form" v-else>
                                <el-form-item label="申请期限(天)：">
                    ...
                    <el-table :data="repaymentList" border fit highlight-current-row style="width: 100%">
                        ...
                        <el-table-column align="center" label="还款方式">
                            <template scope="scope">
                                ...
                                <span v-else-if="scope.row.repaymentMode==='20'">人工</span>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" label="还款金额(元)">
                            <template scope="scope">
                ...
                <div class="filter-container" v-if="lendOrderPo.delayFlay===1">
                    <h2>逾期信息</h2>
                    <el-row>
                        <el-col :span="10" :offset="1">
                            <el-form label-width="200px" class="credit-aplly-form" v-if="listRepaymentPo">
                                ...
                                <el-form-item label="逾期日息(元)：">
                                    {{listRepaymentPo[0].overdueDailyInterest}}
                                </el-form-item>
                            ...
                            <el-col :span="20" :offset="1">
                                <el-form label-width="150px" class="credit-aplly-form">
                                    <el-form-item label="收款金额:" required="">
                                        <el-input :placeholder="lendOrderPo.totalAmount+''" v-model="submitQuery.params.inputAmount" style="width:200px;"></el-input>
                                    </el-form-item>
                                    <el-form-item label="收款时间:" required="">
                                        <el-date-picker v-model="submitQuery.params.inputDate" type="datetime"  :editable="false">
                                        </el-date-picker>
                                    </el-form-item>
                ...
                <el-collapse v-model="collapse">
                    ...
                    <el-collapse-item title="个人信息" name="3">
                        ...
                        <div v-if="customerDetail.userExtendsPo" style="margin-top:20px;">
                            <el-input v-model="customerDetail.userExtendsPo.occupation" class="www-el-label-input" readonly>
                                <template slot="prepend">
                    ...
                    <el-collapse-item title="联系人信息" name="4">
                        <div>
                            <el-table :data="customerDetail.listUserEmergencyContactPo" border style="width: 100%">
                                ...
                                <el-table-column prop="relationName" label="关系"></el-table-column>
...
export default {
    data() {
        return {
            ...
            total3: null,
            input3: '1',
            billlistQuery: { params: { url: 'lendOrder/toWithholdRepaymentOrder', orderNo: this.$route.query.orderNo } },
            submitQuery: { params: { url: 'lendOrder/withholdRepaymentOrder', orderNo: this.$route.query.orderNo, inputAmount: '', inputDate: '', inputImg: '' } },
            shenpilist: { oldCredit: "", result: "", newCredit: "" },
            shenpiAdvice: { refuseReason: "", memo: "" },
            shenpiResult: null,
            lendOrderPo: { realName: '', orderNo: '', userId: '', amount: '', yield: '', periodCnt: '', realLeadTime: '', paymentStatus: '' },
            productPo: null,
            listRepaymentPo: [{}],
    ...
    created() {
        //this.listQuery.params.orderNo=this.$route.query.orderNo;
    ...
    methods: {
        ...
        getbillList() {
            config(this.billlistQuery.params).then(response => {
                //if (response.respCode === "000000") {
                //this.voucherImgUrl = response.data.listPaymentCapitalPo[0].voucherImgUrl;lendOrderPo
        ...
        submit() {
            ...
            if (this.submitQuery.params.inputAmount === '') {
                this.$message({
                    message: '收款金额不能为空！',
                    type: 'warning'
                });
            ...
            if (this.submitQuery.params.inputAmount != this.lendOrderPo.totalAmount) {
                this.$message({
                    message: '请检查收款金额是否与应还金额一致',
                    type: 'warning'
                });
                return;
            }
            if (this.submitQuery.params.inputDate === '') {
                this.$message({
                    message: '收款时间不能为空！',
                    type: 'warning'
                });
            ...
            if (this.submitQuery.params.inputImg === '') {
                this.$message({
                    message: '收款凭证不能为空！',
                    type: 'warning'
                });
            ...
            this.submitQuery.params.inputDate = parseTime(this.submitQuery.params.inputDate);
            config(this.submitQuery.params).then(response => {
                if (response.respCode === "000000") {
                    this.$router.push({ path: "/billManage/repayAbnormalPendingBill" });
                    this.$message({
        ...
        onFileChange() {
            ...
            reader.onload = function(e) {
                that.submitQuery.params.inputImg = this.result.split(',')[1];
